...
class FieldReferenceFrame(Scene):
    def construct(self):
        ...
        f_ref_x = Arrow(ORIGIN, field_rec.get_right() + [-1, 0, 0], buff=0)
        ...
        r_ref_x = Arrow(robot_rect.get_center(), robot_rect.get_right(), buff=0, color=RED)
        ...
        cam_vlength =1
        c0 = Dot(robot_frame.get_corner(UR), color = GREEN)
        c0_ref_x = Arrow(c0.get_center(), 
            c0.get_center() + [cam_vlength * math.cos(math.radians(22.5)), cam_vlength * math.sin(math.radians(22.5)), 0], 
            buff=0, 
            color=GREEN
        )
        c0_x_lbl = MathTex("X_c -", font_size = 25, color=GREEN).move_to(c0_ref_x.end + [.2, .2, 0])

        c0_ref_y = Arrow(c0.get_center(), 
            c0.get_center() + [cam_vlength * math.cos(math.radians(90 + 22.5)), cam_vlength * math.sin(math.radians(90 + 22.5)), 0], 
            buff=0, 
            color=GREEN
        )    
        c0_y_lbl = MathTex("Y_c -", font_size = 25, color=GREEN).move_to(c0_ref_y.get_top() + [0, .2, 0])
    
        camera0 = VGroup(c0, c0_ref_x, c0_x_lbl, c0_ref_y, c0_y_lbl)
        c2 = Dot(robot_frame.get_corner(DR), color = GREEN)
        c2_ref_x = Arrow(c2.get_center(), 
            c2.get_center() + [cam_vlength * math.cos(math.radians(-22.5)), cam_vlength * math.sin(math.radians(-22.5)), 0], 
            buff=0, 
            color=GREEN
        )
        ...
        c2_ref_y = Arrow(c2.get_center(), 
            c2.get_center() + [cam_vlength * math.cos(math.radians(-90 + -22.5)), cam_vlength * math.sin(math.radians(-90 + -22.5)), 0], 
            buff=0, 
            color=GREEN
        )    
        c2_y_lbl = MathTex("Y_c +", font_size = 25, color=GREEN).move_to(c2_ref_y.end + [0, -.2, 0])
    
        camera2 = VGroup(c2, c2_ref_x, c2_x_lbl, c2_ref_y, c2_y_lbl)


        robot_full = VGroup(robot_frame, camera0, camera2).rotate(-PI/6, about_point=robot_frame.get_center())
